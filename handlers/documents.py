...
async def save_voice_for_transaction(context: ContextTypes.DEFAULT_TYPE, file_id: str, transaction_num: int, user_id: int):
    """Сохраняет Telegram file_id аудио и прикрепляет ссылку к транзакции."""
    try:
...
async def handle_incoming_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        file_link = f"https://api.telegram.org/file/bot{TELEGRAM_BOT_TOKEN}/{file_obj.file_path}"
